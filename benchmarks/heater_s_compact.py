"""Compact eight-input, four-lane MMI-heater-MMI-heater benchmark.
...
Compared with ``heater_s``, the output-side heater/MMI array uses half the
extra horizontal spacing, so the final elements sit closer to the upstream
combiner tree while keeping enough run length for path-length matching.
"""
...
def build_schematic() -> Schematic:
    ...
    output_array_internal_gap_x = 500
    pre_output_heater_shift_x = 500
