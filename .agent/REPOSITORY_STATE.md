...
- **Chip-boundary keepout landed (plan `.agent/execplans/2026-09-04-chip-boundary-keepout.md`):** routes can no longer leave the chip. The routable die is the component bbox plus the explicit `chip_add_x/y_um` margins (flow defaults 0 / 40 um); the `security_margin_um` grid padding (20 um) is static -- four bands from `static_obstacle_builder.chip_boundary_keepout_rects`, handed to Rust via `PyPhotonicRouter.add_static_rects`; the final photonic gate rejects any centerline outside the routable bbox (`route_outside_chip`, metric `outside_chip_route_count`). Cause of the owner's finding (a multiportmmi_32x32 route east of the output couplers): the padding was free space. All seven benchmarks verification-clean with unchanged verdicts; mm32 119 -> 121 crossings (the detour became two legal crossings). Open for the owner: whether `chip_add_y_um = 40` should also be 0.
