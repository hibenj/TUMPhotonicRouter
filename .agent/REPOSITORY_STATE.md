...
- **Three switchable configurations (owner rule):** baseline `--crossing-mode lidar-pure` (benchmark stable flags) / contribution 1 `--crossing-mode lidar-guided` (+ optional `PHOTONIC_ROUTER_PLANNED_CROSSING_BUDGET=1`, `--net-order plan-crossings-desc`) / contribution 2 `--preplaced-crossing-grids true`; the flow rejects 1+2.
