...
- **2026-09-24, branch `restructure/modular-engine` (Milestone 7 of `.agent/execplans/2026-09-22-modular-readable-router-restructure.md`):** the structure is written down. `docs/ARCHITECTURE.md` is the module map of Rust and Python (one paragraph per module, what it owns and what it must not know), the flow as the literal nine stage calls of `translation/routing/session.py::run` with each phase's input and output type, the `NetSearch` interface with the recipe to add an engine, the negotiated loop with its four policies and the recipe to add one, the configuration objects and their precedence (defaults < benchmark stable block < environment < command line), the three paper configurations, the build/test commands, the Milestone 8 removal candidates with module and size, and the 19 shared-writer fields of `SessionState`. `docs/CONFIGURATION.md` is generated from the dataclasses by `scripts/generate_config_reference.py` (88 `RoutingConfig` fields with their overlay variables, 48 `FlowOptions` fields with their command-line flags) and pinned by `tests/test_config_reference.py`. `AGENTS.md` and `README.md` now describe the current layout and point at both documents; the usage examples are `python -m photonic_router route <benchmark> --configuration ...` with the script form noted as still working. Documentation only, no behaviour change. Plan status: Milestones 0-6 done, Milestone 7 documents done and the full 27-cell reproduction pending, Milestone 8 (the negotiated removals) open.

- **2026-09-22, branch `DATE2027-results` (owner go, steps 1-2 of the reproduction plan):** one branch that regenerates every row of ours in the paper table. Kernel = frozen engine `8ddde83` plus only the contribution 2 long-straight exemption of `a181115` (`git diff 8ddde83 -- src/py_router.rs` is those 42 lines); Milestone 9 (`39ea78d`, span-scaled budgets) is absent. `routing_flow.py` keeps enabling the exemption under `--preplaced-crossing-grids`, so contribution 2 stays one flag; Benes has no dense instances (no-op there). Tooling: `run_and_archive.sh` honours `RESULTS_ROOT`; `scripts/results/reproduce_date2027.sh` (all rows, archives in `results_date2027/`), `scripts/results/compare_date2027.py` (exact on crossings and GDS length, zero verifier errors, time as a ratio; self-checked against the paper archives: all ok); provenance per row in `docs/DATE2027_REPRODUCTION.md`. Validation: kernel rebuilt (`RUSTUP_TOOLCHAIN=stable-x86_64-unknown-linux-gnu`, `PYO3_PYTHON=.venv/bin/python` for `cargo test`), negotiated Rust tests 7/7, `pytest` 11 failed / 411 passed = the long-standing baseline (same 11 fail on the frozen kernel with identical error text). Step 3 (owner go, 2026-09-22 12:52-13:49): Benes ladder x 3 configurations and the five mesh contribution 2 rows reproduced, 17/17 cells exact on crossings and GDS length, 0 verifier errors, loop-time ratio 0.92-1.13 (archives in `results_date2027/`, `docs/DATE2027_REPRODUCTION.md` Verification). The mesh contribution 2 rows match without Milestone 9, so it stays absent. Remaining 10 cells (owner go, 13:50-16:59): also exact, so all 27 cells of ours reproduce (loop-time ratio 0.92-1.14); comparison table in the branch document. `results_date2027/` (109 MB, untracked like `results/`) holds the reproduction archives. The paper is final and is not touched.
...
- **ADEPT 128x128 with contribution 2 routes (2026-09-17):** the long-straight congestion penalty is off for the nets of dense multi-port instances (`PyPhotonicRouter::long_straight_exempt_net_ids`, filled from the static fan-out anchors, default on with `--preplaced-crossing-grids`, env `PHOTONIC_ROUTER_LONG_STRAIGHT_EXEMPT_DENSE_FANOUT=0` to disable); with it every contribution 2 row completes: ADEPT 128 in 638 s wall / 352 s A*, 311 crossings = plan, 0 repairs; the ADEPT rows of contribution 2 were rerun (Benes rows have no dense instances and are unchanged). Milestone 9 of the negotiated-engine plan (span-scaled budgets, retry after rip-up, retry behind the probe) is in the default engine; Benes 64 stays open (stage packing of the last net of a shuffle stage). Paper table regenerated by `scripts/results/paper_table.py results main.tex`; KEY_FINDINGS 4f and EXPERIMENTS_TEXT_NOTES.md updated.
- Date: 2026-09-16 (afternoon)
- **Baseline frozen (owner decision 2026-09-16 16:50):** the negotiated rip-up engine is the default repair engine for every mode (`negotiated_repair_engine_enabled` in `translation/route_rust.py`; `PHOTONIC_ROUTER_LEGACY_REPAIR_CHAIN=1` or `PHOTONIC_ROUTER_NEGOTIATED_REPAIR=0` restores the old chain for A/B); braid escalation off by default (`PHOTONIC_ROUTER_NEGOTIATED_BRAID_ESCALATION=1` enables); lidar-pure keeps declaration order, contribution 1 the hybrid order plus the crossing-free rule, contribution 2 the span order with the original-netlist depth map. Full archived rerun `scripts/results/rerun_all.sh` (ladder x 3, mesh64 x 3, benes64/benes128 contribution 2, then original LiDAR at price 0) started 16:55; results in `results/`, log `results/rerun_all.txt`. **Rerun complete 2026-09-17 08:54:** all 26 runs of ours clean (0 verifier errors, every crossing count at plan except lidar-pure mm64 = 217), contribution 2 rerun with `--verbose-routes` for route lengths, LiDAR price 0 (mesh64 5112 s, 224 X) and price 300 (mesh16 crash, benes32 4 h timeout, mesh64 4 h timeout in iteration 9) archived; paper table = KEY_FINDINGS.md section 4f (`scripts/results/table.py results`). benes_64x64 for lidar-pure / contribution 1 and multiportmmi_128x128 for contribution 2 stay open (timeouts / no convergence in the 5-um bands).
- Date: 2026-09-16 (morning)
- **Archive rule (owner decision 2026-09-16):** every paper-relevant run is stored per configuration under `results/<benchmark>/<config>/<timestamp>/` by `scripts/results/run_and_archive.sh` (private working directory per run, then GDS + both verification JSONs + log + `run.txt` + `metrics.json` via `scripts/results/summarize_run.py`: waveguide lengths, crossing counts, verifier errors). `results/` is gitignored. The final benchmark-quality table (waveguide length, crossings per benchmark) comes from a full rerun of all configurations through this script.
- Date: 2026-09-16 (night)
- **Contribution 1 realizes the 64x64 mesh plan exactly; braid analysis (2026-09-15/16, plan `.agent/execplans/2026-09-14-lidar-style-negotiated-ripup-endgame.md` Milestones 7-8):** Milestone 7 = crossing-free search for nets without a planned crossing under `lidar-guided` (`negotiated_crossing_free_net`, router flag `negotiated_crossing_free_search`, env `PHOTONIC_ROUTER_NEGOTIATED_CROSSING_FREE_UNPLANNED=0` to disable): `multiportmmi_64x64 --crossing-mode lidar-guided` clean in 20 min with 209 crossings = plan (was 217 / 38 min); slice `multiportmmi_64x64_bands1to2` (new, `benchmarks/`) 0 crossings. Milestone 8 = braid escalation (`BraidRepairOutcome::VictimRipped`, `requeue_braid_victims`): a braid partner that cannot be rerouted after the swap is handed back to the queue instead of rolling back to the braid; measured neutral-to-negative for lidar-pure (mesh 218 vs 217: the re-queued net's priced search takes single crossings instead of failing). Root cause of the fan-out braids is the net order (first net of a splitter pair boxes the second); LiDAR orders a level shortest-first (`routing_order = int(EulerDist)`), our `--net-order topological-span` reproduces that and removes all 8 unplanned crossings on the slice; on the full mesh it routes with 0 unplanned crossings in 85 min but ends in an endpoint-correction error on n_889 (open). Ladder both modes on the new build: 14/14 at plan. Owner decision pending: lidar-pure's default order. Trace additions: `native_repair_braid_geometry/_neighbours`, `native_repair_victim_reroute_failed`, `crossing_free=` fields. `cargo test --lib` 484/484.
- Date: 2026-09-15 (evening)
- **The loss-driven baseline routes the 64x64 mesh (2026-09-14/15, plans `.agent/execplans/2026-09-14-loss-driven-endgame-at-64x64.md` and `.agent/execplans/2026-09-14-lidar-style-negotiated-ripup-endgame.md`):** `multiportmmi_64x64 --crossing-mode lidar-pure` with `PHOTONIC_ROUTER_NEGOTIATED_REPAIR=1` routes clean in 58 min (A* 3441 s), 217 crossings = 209 planned + 8 braids, 0 errors; LiDAR (its comparison config, crossing search price 0) needs 89 min and 224 crossings; the old repair chain fails at 59 min. The 14-run ladder (benes 4-32, meshes 8-32, both lidar modes) realizes exactly the topology plan on every benchmark under the negotiated engine, 0 errors. What changed: (1) the negotiated repair engine (`route_many_with_negotiated_repair_and_commit`, still opt-in via the env var; Milestone 6 = making it lidar-pure's default is an open owner decision) now mirrors LiDAR's loop -- local rip-up of the illegal-crossing partners of a failed net with re-queue (once per epoch), global rounds ripping remaining failures with their last-probe blockers under history cost (history cleared once, at the second global rip-up), up to 10 rounds, every commit fully legal; fail-fast budgets (2 M first attempt, one 10 M retry, 30 M for failed nets, full in the last round; `AStarConfig.total_expansion_budget` bounds the whole search call including every window); probe first, then direct crossing on at most three of the probe's illegal partners, no clean-probe commit; braid repair over every braided partner after each commit; a **probe-guided search**: when the priced search has failed twice, the probe's crossing partners plus every committed net of their source columns are priced at 0 for one search (contribution 1's guidance mechanism fed from geometry instead of the plan) -- the root cause of the 64x64 stall was the A* cost contour: with a 200 um crossing price and an admissible heuristic the search sweeps every rung of a fan-out bundle before paying for the next crossing; (2) two engine-independent guards: a commit is rejected when the route's centerline intersects a committed net without a registered crossing event (`missing_crossing_event`; the probe's grid-based reconstruction missed corner intersections of two diagonals -- now geometric), and the verifier's cross-net overlap threshold is 0.05 um2 instead of 2.0 (a full 0.25 um2 crossing overlap was invisible); (3) the chain's coarse column repair defers unrestorable nets instead of aborting; (4) the dense-grid cap of the crossing search is sized from the obstacle map (the 64x64 dies exceeded the 10 M default and panicked); (5) a timing trace for the negotiated loop (`t=`, per-net and per-search lines, `native_negotiated_done`). Tooling: `scripts/make_mesh_slice_benchmark.py` cuts a self-contained column range out of a mesh (`multiportmmi_64x64_bands4to5`, `_bands8to9`, `_bands6to9`, `_bands7to9`): the 8to9 slice reproduces the net-575 stall in 15 min and is the regression case; `benchmarks/benes_128x128.py` and `multiportmmi_128x128.py` exist, not yet run. Facts for the paper: LiDAR's `default_config.yml` prices crossings at 5000 (its units; ~3.3 mm of waveguide) and its `comp_LiDAR.yml`, used for every LiDAR run here, at 0 with `il_cross 0.5` dB for evaluation only; LiDAR times out at 12 h on benes_64x64. Weighted A* stays ruled out (2026-08-31). Open runs: benes_64x64 lidar-pure and contribution 1 on both 64x64 cases under the negotiated engine, contribution 2 on 128x128, LiDAR reruns at our price (loss_crossing 300 in LiDAR units = 200 um at loss_propagation 1.5).
- Date: 2026-09-07 (morning)
- **64x64 scaling benchmarks + LiDAR bridge (2026-09-10/11, plan `.agent/execplans/2026-09-10-64x64-scaling-benchmarks.md`, uncommitted):** `benchmarks/benes_64x64.py` (stage pitch 1500 um: same room per swap level as the 32x32; 1824 planned crossings) and `benchmarks/multiportmmi_64x64.py` + `benchmarks/data/multiportmmi_64x64.yml` (generated with LiDAR's own `MMIports.py` at double the 32x32 die area, seed 1234; not a published LiDAR benchmark). One-hour runs: LiDAR, our baseline and contribution 1 all time out on both 64x64 benchmarks (contribution 1 with 0 failures until the timeout, benes at route 670/768, mesh at 795/895); contribution 2 routes the 64x64 mesh clean in 116 s (209 tiles) and the 64x64 Benes clean in 93 s (4416 routes, 1824 tiles, 0 failures) after the X-tile snap fix of 2026-09-11: tile centres are snapped to routing-grid cell boundaries so a port's cell is the footprint's edge cell (the router's diagonal halo rule rejected the last 45-degree step when the tile sat mid-cell; `_snap_x_tile_center`). Grid ladder identical afterwards. LiDAR bridge `scripts/lidar_bridge/{export_benes_spec.py, benes_from_spec.py}` exports our Benes networks (switches expanded into MMIs + heaters, internal nets included) into LiDAR's YAML via LiDAR's own writer: LiDAR routes benes_8x8 in 21 s and benes_16x16 in 1363 s with DRV 0 and exactly the planned crossings, and times out on 32x32 and 64x64 (our baseline: 997 s clean on 32x32). Memory rule (owner): a run stopped for memory is recorded as out-of-memory = failure; a watchdog was used, never fired.
- **Contribution 1 default = S1 + hybrid net order (2026-09-08, owner decision, plan `.agent/execplans/2026-09-04-crossing-guided-search.md`):** `--crossing-mode lidar-guided` now resolves `net_order=None` to `plan-crossings-hybrid` (`default_net_order(guided=True)`); lidar-pure and grid mode unchanged. Ladder with the bare flag reproduces the hybrid table exactly (mm8 -18 %, benes8 -95 % and 0 failures, mm16 -23 % and 63 crossings, benes16 -45 %, mm32 identical), all verifications 0 errors.
- **Contribution 2 = general geometry builder (2026-09-08, plan `.agent/execplans/2026-09-08-crossing-structure-builder.md`, commits `abc8e1a` selector + `60c672c` fallback):** per crossing layer `translation/crossing_structures.select_layer_structure` evaluates every structure family (X array for opposite-direction pairs; axis-aligned column grid for lanes on the router's fan-out anchors) for feasibility (band, port geometry, column budget, cyclic order) and cost (extra length over octile, corners, tiles, band used) and takes the cheapest feasible one; the decision per layer is printed (`crossing structure layer a->b: ...`) and recorded in the metrics (`layer_structures`). A layer with no feasible structure is a ROUTER fallback: its nets stay whole and the flow runs the optical stage with crossings enabled in `lidar-guided` mode with the plan guidance restricted to those nets (`crossing_guidance_net_names`), so the tiled layers stay crossing-free; verification enforces tiles + router-realized crossings == `plan_event_count`. Forced check on multiportmmi_8x8 (`PHOTONIC_ROUTER_CROSSING_GRID_ROUTER_LAYERS=6`): 26 tiles + 7 router crossings = 33 of 33, both verifications 0 errors (`tests/test_router_fallback.py`). Seven-benchmark grid ladder unchanged (all layers get a tile structure, 0 failures / 0 repairs / 0 errors). `CrossingGridGeometry(router_fallback=False)` restores the hard stop.
- **Contribution 2 works on every Benes size (plan `.agent/execplans/2026-09-07-preplaced-crossing-grids-all-benes.md`, uncommitted working tree on `crossings/verification-foundation`):** `routing_flow.py benes_<size> --preplaced-crossing-grids true` alone routes 4x4 / 8x8 / 16x16 / 32x32 verification-clean (benes_32x32: 516/516 stubs, 0 failures, 0 repairs, 30 grids / 416 crossing components = topology count, 30 s; lidar-pure baseline of the same benchmark ~15 min). Three defaults were the gap, none of them router rules: crossing-grid `lane_pitch_um` 20 -> 14 (at 20 the 32x32 stage-1 grid is 482 um of the 544.5 um band, 31 um per side for the 32-lane fan-in; at 14 it is 392 um); the flow selects `--net-order topological-span` for grid mode (planar stubs; declaration order fails 16x16 and 32x32), explicit order wins; a benchmark's stable block loses its `--crossings/--crossing-mode` flags under grid mode. Guards: pytest 380 passed / same 10 baseline failures; benes_8x8 and mm8 lidar-pure verdicts identical. Not applicable to multiportmmi as built (the lattice rejects lanes that reverse direction: owner scope question). GDS `build/routed_benes_*_grid_full.gds`.
- **Side item 3 (2026-09-07 21:30): `--net-order plan-crossings-hybrid`** (desc in layers of <= 16 nets, declaration order in larger ones) -- ladder: benes8 -95 % expansions and 0 failures, mm16 -23 % with the braid gone, benes16 -45 %, mm8 -18 %; 32x32 cases identical to S1; opt-in, owner decides whether it becomes contribution 1's default (it changes the ladder layouts: mm16 65 -> 63 crossings). Table in the contribution 1 plan.
- **Side items after the paper base (2026-09-07 evening):** (1) contribution 1 re-measured on the paper base: every expansion count and verdict bit-identical to the 2026-09-04 A/B (table appended to `.agent/execplans/2026-09-04-crossing-guided-search.md`); (2) photonic verifier prefilters (`.agent/execplans/2026-09-07-verifier-prefilter.md`): bbox prefilter for the cross-net check and the residue-based per-route obstacle check, no verdict changes; grid-mode flow totals benes_32x32 107 -> 15 s, multiportmmi_32x32 84 -> 24 s. Still open: contribution 1 net-order follow-up (hybrid order / benes32 net 282), and the column grid's second-column extension for arbitrary permutations between widely spaced port columns.
- **Baseline ladder re-verified after the day's changes (2026-09-07 19:30, lidar-pure stable defaults, sequential except benes_32x32 in the background):** heater_s_mod 81/0/0; multiportmmi_8x8 111/0/0, 33 X; benes_8x8 52/4/0, 16 X; multiportmmi_16x16 227/2/1, 65 X; benes_16x16 136/8/0, 88 X; multiportmmi_32x32 457/0/5, 121 X, 307 s; benes_32x32 340/16/2, 416 X, 997 s -- every verdict identical to the 2026-09-04 records, all verifications 0 errors. The three configurations (baseline lidar-pure / contribution 1 lidar-guided / contribution 2 preplaced crossing grids) are all switchable and clean; committed the same evening.
- **Contribution 2 generality sweep + corner pre-wiring (2026-09-07 evening, same plan):** synthetic permutation benchmark `benchmarks/permutation_synthetic.py` + `scripts/path_investigation/synthetic_grid_sweep.py` (70 configs, results in `.agent/execplans/2026-09-07-synthetic-sweep-results.md`): every accepted configuration routes 0/0 clean; rejections are loud (band too narrow for the columns; random permutations between widely spaced single-port columns are cyclic for the one-column-per-lane family). Rules changed on the evidence: dense (2-10 um) port groups -> column grid; column sharing respects order; ALL corners of column-grid lanes pre-wired (crossing-with-bend tiles or bare bends, default `PHOTONIC_ROUTER_CROSSING_GRID_CORNERS=always`); hops < 10 um are static links registered without port lanes. Do NOT set `PHOTONIC_ROUTER_FANOUT_LANE_SPACING_CELLS` explicitly: even its default value changes router behaviour. Final ladder: all seven benchmarks 0 failures / 0 repairs / 0 errors (mm8 224 nets 7 s, mm16 452 nets 19 s, mm32 916 nets 84 s).
- **Contribution 2 COVERS ALL SEVEN BENCHMARKS (2026-09-07 15:40, plan `.agent/execplans/2026-09-07-distributed-crossing-structure.md`, uncommitted):** `--preplaced-crossing-grids true` alone routes benes 4/8/16/32 and multiportmmi 8/16/32 verification-clean. Structure = crossing tiles only, the normal router connects: Benes layers use X tiles on level columns (all pairs opposite-direction); multiportmmi layers use the axis-aligned column grid (`_column_grid_stage`): every lane horizontal on its static fan-out anchor row (probe `route_rust.static_fanout_anchors_um`, per-instance two-port stubs via `PHOTONIC_ROUTER_DENSE_FANOUT_INSTANCES`) -> vertical in its column -> horizontal on its target row; "+" tiles where a vertical meets a horizontal; column order from the pairwise crossing requirements with an 11 um corner clearance (soft), widest travel first, column sharing that respects the order, 16 um pitch, static links between tiles closer than 8 um. Numbers: mm8 177 routes 0/0 in 7 s, mm16 349 routes 0/0 in 15 s, mm32 687 routes 3 failures / 3 repairs in 57 s (lidar-pure 14 / 45 / ~300 s); waveguide length within 2 % of lidar-pure. Benes unchanged (0 failures, 0 repairs). Open: photonic verifier time (benes32 108 s wall for 4 s of routing), owner review of the pictures in `build/crossing_structure_samples/`.
- **Contribution 2, third design LANDED 13:30 (plan `.agent/execplans/2026-09-07-distributed-crossing-structure.md`): crossing TILES are the default grid mode** (`fan_mode="tiles"`, owner: pre-place only the crossings, the normal router connects them): one 45-degree crossing tile per planned crossing at the stretched-array position, lanes split into `__to_grid` / `__via<i>` / `__from_grid` router nets; all four Benes sizes 0 failures / 0 repairs / 0 errors, benes_32x32 1152 routes, A* 3.9 s, waveguide 241.2 mm vs lidar-pure 234.1 (compact lattice 294.2); flow total 104 s because the photonic verifier spends 94 s on 1152 routes x 416 instances (follow-up). Previous design of the day, still selectable (`PHOTONIC_ROUTER_CROSSING_GRID_FAN_MODE=stretched`): the pre-wired stretched array: crossings at the midpoint of the two rows they swap, swap levels as columns spread over the band, lanes = 45-degree arms through the crossings joined by diagonals/verticals/horizontals; band margin 50 um for the router's port-to-slot stubs. Ladder: all four Benes sizes 0 failures / 0 repairs / 0 verification errors; benes_32x32 516/516 in 17 s, waveguide length 248.6 mm vs lidar-pure 234.1 / compact lattice 294.2. `PHOTONIC_ROUTER_CROSSING_GRID_FAN_MODE=router` restores the compact lattice. multiportmmi stays open (reversing lanes, same-direction crossings). Pictures in `build/crossing_structure_samples/`.
- **Owner direction (2026-09-07):** all contributions must work first; contribution 1 = S1 (`--crossing-mode lidar-guided`, already its default shape: budget 0, topological order); contribution 2 next (done above); then return to contribution 1 improvements (S3 hybrid / net 282).
- Previous snapshot (2026-09-04 session close):
- Date: 2026-09-04 (session close, ~21:40)
- **Resume point:** HEAD `fc033be` on `crossings/verification-foundation`, working tree clean, no flow process, deployed `python/photonic_router/_rust.abi3.so` = build of HEAD (last Rust change S2, 17:33). Baseline tag `baseline-2026-09-03` (pre-predicate-1). Every result of the day is in the ExecPlans: `2026-09-04-chip-boundary-keepout.md` (COMPLETE), `2026-09-04-crossing-guided-search.md` (contribution 1: S0/S1/S2/S3 all landed, S2 off and S3 opt-in by owner decision / evidence), `2026-09-04-mm32-first-layer-no-crossings.md` (COMPLETE), `2026-09-04-predicate-1-one-number.md` (PARKED). Reusable scripts saved to `scripts/path_investigation/` (`gds_outside_chip.py`, `plan_vs_realized_crossings.py`, `ab_table_template.py`); the scratchpad logs/GDS copies of the A/Bs are session-local (`build/routed_<bench>_{lidar-pure,lidar-guided}.gds`, `..._chipy0_full.gds`, `..._guided_plan-crossings-desc.gds` remain in `build/`).
- **Open owner questions (collected, none blocking):** (1) S3 follow-up: hybrid net order (desc only where the planned-crossing pack stays sparse / interleaving) or a harness investigation of benes32 net 282 under `plan-crossings-desc`; (2) predicate-1 step 2 rows A-D (parked); (3) `min_straight_cells_per_crossing` dead knob -- delete or make it the half_size override; (4) P4 structural search cost (Tier-2 key multiplicity, heuristic lower bound); (5) push the tag/branch to the remote if wanted; (6) the mm32 output-permutation layer (n_430/n_441 cross 10-11 nets each) is untouched by any contribution so far.
- **Three switchable configurations (owner rule):** baseline `--crossing-mode lidar-pure` (benchmark stable flags, `topological` net order) / contribution 1 `--crossing-mode lidar-guided` (default net order `plan-crossings-hybrid` since 2026-09-08, owner decision; `PHOTONIC_ROUTER_PLANNED_CROSSING_BUDGET=1` and `--net-order plan-crossings-desc` stay opt-in) / contribution 2 `--preplaced-crossing-grids true` (`topological-span`); the flow rejects 1+2.
- **Contribution 1 S2/S3 (evening):** S2 per-pair budget (`PHOTONIC_ROUTER_PLANNED_CROSSING_BUDGET`, default 0 = off: identical layouts, +7.5 % search on benes32). S3 `--net-order` (`translation/route_order.py`: topological | topological-span | plan-crossings-desc/asc; default topological; the old `PHOTONIC_ROUTER_LAYER_ORDER=span` env is gone). `plan-crossings-desc` under lidar-guided: mm8 -18 %, benes8 -95 %, mm16 -23 % (braid gone, 63 = plan minimum), benes16 -45 % expansions, fewer failures; mm32 neutral; **benes32 middle stage hangs (net 282 > 3 min, watchdog)** -> opt-in only. Open: hybrid order or net-282 investigation.
- **Contribution 1 LANDED (`a94750b`, plan `.agent/execplans/2026-09-04-crossing-guided-search.md`): `--crossing-mode lidar-guided`** = lidar-pure mechanics + the topology plan as soft search guidance (planned pairs cost `PHOTONIC_ROUTER_PLANNED_CROSSING_SEARCH_LOSS_UM` = 0 in the search, unplanned ones keep the 200 um collision price; the engine may still add crossings). A/B on the six crossing benchmarks (same build, back to back): identical attempts/failures/repairs and crossing counts, route length within +-0.05 %, **34-45 % fewer A* expansions** (benes32 977 -> 655 s search, mm32 269 -> 139 s), both verifications 0 errors. Baseline untouched (empty guidance == baseline pricing, kernel test). Design history: precomputed topology crossings as A* guidance. Measured first: the topology plan predicts the realized lidar-pure crossing set EXACTLY on all six crossing benchmarks (mm16 +1 braid); the multiportmmi metadata ships empty depth/rank dicts that break plan construction (`{}` treated as explicit) -- slice S0. Owner rule: baseline lidar-pure / contribution 1 (`--crossing-mode lidar-guided`) / contribution 2 (`--preplaced-crossing-grids`) are three mutually exclusive, switchable configurations; contributions never leak into baseline defaults.
- **Chip-boundary keepout landed (plan `.agent/execplans/2026-09-04-chip-boundary-keepout.md`):** routes can no longer leave the chip. The routable die is exactly the component bbox unless `--chip-add-x/y-um` (flow defaults now 0 / 0, owner decision) or `--die-bbox` widen it; the `security_margin_um` grid padding (20 um) is static -- four bands from `static_obstacle_builder.chip_boundary_keepout_rects`, handed to Rust via `PyPhotonicRouter.add_static_rects`; the final photonic gate rejects any centerline outside the routable bbox (`route_outside_chip`, metric `outside_chip_route_count`). Cause of the owner's finding (a multiportmmi_32x32 route east of the output couplers): the padding was free space. All seven benchmarks verification-clean with unchanged verdicts; mm32 119 -> 121 crossings (the detour became two legal crossings). All seven layouts lie entirely inside their component bbox (klayout scan).
- **Braid repair landed (`a1ffa32`, plan `.agent/execplans/2026-09-04-mm32-first-layer-no-crossings.md`):** the multiportmmi_32x32 first layer (fanout -> MZI -> first heaters) is crossing-free like the LiDAR reference. Cause of the former 4 crossings: greedy sequential routing in the wrong pair order (the earlier net's shortest path sealed its sibling's target pocket / walled the neighbouring branch), not halos, congestion or cost. Rule: a net crossing the same partner twice is rerouted with the pair order swapped, kept only if the crossings between them decrease. Guard: every benchmark routes verification-clean; mm32 149 -> 119 crossings (5 repairs), benes32 420 -> 416 (2). Baseline tag `baseline-2026-09-03` = the state before predicate 1; predicate 1 is PARKED after its step 1 (difference matrix as tests, rule code = baseline; `.agent/execplans/2026-09-04-predicate-1-one-number.md`, branch `pred1-rework-wip`).
- Previous snapshot (2026-09-03 evening):
...
Owner priority 2026-09-16: a fixed baseline for the paper first, optimizations afterwards (numbers updated only if they pay). In order:

1. **multiportmmi_128x128 with contribution 2**: the first heater-to-MMI net (n_1643, `mmi1_ps_array_1_heater_108,o2 -> mmi1_multiport_1_0,o109`) finds no route; `PHOTONIC_ROUTER_SEARCH_FAILURE_DIAG` reports 99 static blockers that lie in free space about 190 um in front of the MMI input face (not inside any instance bbox) -- so static stubs / keepouts / the static-obstacle construction, or the generator's geometry. Identify the owner of those cells, fix or regenerate, rerun (`scratchpad/night2/mm128_diag.log`, `.agent/execplans/2026-09-10-64x64-scaling-benchmarks.md`).
2. **Baseline decision (Milestone 6 of the negotiated-engine plan)**: negotiated engine as default for the lidar modes, declaration order kept; re-measure the ladder for the paper.
3. **Optimizations, each A/B against that baseline**: LiDAR's shortest-first order inside a level (`--net-order topological-span`; 0 unplanned crossings on the 64x64 mesh but an endpoint-correction error on n_889 to fix first); braid-escalation default for lidar-pure (`PHOTONIC_ROUTER_NEGOTIATED_BRAID_ESCALATION`, measured 218 vs 217); halo-aware probe for the Benes 64x64 first stages.
