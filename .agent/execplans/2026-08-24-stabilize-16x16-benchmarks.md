...
- [x] (2026-08-24) Milestone 0: re-verified `benes_8x8` and `benes_16x16` are still clean after all restructuring since the 2026-08-18 baseline. `benes_8x8`: `error_count=0, warning_count=0` both verifications, 22.2s. `benes_16x16`: `error_count=0, warning_count=0` both verifications, 252.9s (~4.2 minutes) -- a concrete performance data point, not yet investigated further, ahead of any planned performance work.
  **Correction (2026-08-24, later the same day, found and fixed while validating `.agent/execplans/2026-08-24-crossing-cost-function-soundness.md`'s Milestone 3):** the runs above used the wrong configuration. Both `benes_8x8` and `benes_16x16` document their own "Stable crossing-router baseline" (`benchmarks/benes_8x8.py:23-25`, `benchmarks/benes_16x16.py:23-25`): `--crossings true --crossing-mode lidar-pure`. The bare-defaults command used above does not supply this and is not representative -- caught by the repository owner directly. Re-verified with the correct configuration: `benes_8x8` (`--crossings true --crossing-mode lidar-pure`, `error_count=0, warning_count=0`, `25.2s`) and `benes_16x16` (same flags, `error_count=0, warning_count=0`, `392.4s` -- the first real timing baseline for this configuration; the `252.9s` figure above is not a valid comparison point). Also see `.agent/REPOSITORY_STATE.md`'s corrected "Re-verified clean" entry.
  **Also found while validating the same plan (2026-08-24), relevant to this plan's own paused Milestone 1:** `multiportmmi_16x16` stable-baseline's failure signature has moved. It no longer fails on `n_50` (`candidate_blockers=[49, 50]`, the finding this plan's Context and Orientation section documents in full) -- it now fails on `n_49` (`candidate_blockers=[49]`, a related but distinct net, different `recent_errors` trace: `RuntimeError: No route found for n_49: mmi0_ps_array_0_heater_2,o2 -> mmi0_multiport_0_0,o3. ... error=No repair route found; candidate_blockers=[49]; recent_errors=["reroute_victims:net49:roundSome(1):rip[49]:No route found", "reroute_victims:net49:roundSome(1):rip[49]:No legal LiDAR crossing route found", "repair_failed_net:net50:roundSome(1):rip[49]:No legal LiDAR crossing route found"]`). This is plausibly caused by the crossing-cost-function-soundness plan's weight/cost changes shifting routing order or the specific congestion point in this tightly-packed net cluster (`n_49`/`n_50` were already flagged together as a `candidate_blockers` pair in the original finding). Not root-caused further here, per repository owner instruction to finish that plan and park further investigation. Whoever resumes Milestone 1 below needs to re-establish which net (or both) is the actual root blocker before continuing.
