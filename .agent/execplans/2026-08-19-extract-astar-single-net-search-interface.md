...
- [x] Milestone 2 (implementation) complete, both halves done and reviewed. `CrossingSearchConfig` dedup half completed by Codex on 2026-08-19: new `fn crossing_search_config` helper added next to `astar_config` (`src/py_router.rs:2811`), all 3 duplicate call sites migrated to call it via `crossing_loss_override`/`require_all_partners_override` parameters. **Reviewed by Claude**: diff read in full, all 3 sites confirmed to resolve to field-for-field identical `CrossingSearchConfig` values as before (site 1: `crossing_loss_override` passed through, `require_all_partners_override: None` -> `crossing_cfg.allow_only_expected_pairs`; site 2: `None`/`Some(true)`; site 3: `None`/`Some(require_all_expected_partners)` -- all three match the original struct literals exactly). `cargo test --lib` independently re-run: `334 passed, 0 failed` (unchanged, as expected for a pure refactor with no new tests). `maturin develop --release` succeeded. Full `pytest -q` independently re-run: `21 failed, 325 passed, 1 skipped` -- identical to the pre-change baseline. Committed.
- [x] Milestone 3 (broaden test coverage): already substantially satisfied by Milestone 2's 4 trait-equivalence tests (one per mode, each directly exercising the new interface boundary) -- no additional tests needed, same conclusion as the grid-snapping plan's own Milestone 3.
- [x] Milestone 4 (broad validation) complete. `cargo test --lib` `334 passed, 0 failed`. `PYTHONPATH=. .venv/bin/pytest -q` `21 failed, 325 passed, 1 skipped` (byte-identical to pre-plan baseline). `benes_4x4`: **Verdict: PASS** (`error_count=0, warning_count=0`, both crossing and photonic verification). `multiportmmi_8x8` bare CLI defaults: fails with the exact same pre-existing `RuntimeError: No route found for n_70` signature already root-caused and parked -- confirmed not a regression. `multiportmmi_8x8` stable-baseline config: **Verdict: PASS** (`error_count=0, warning_count=0`, both crossing and photonic verification). Every verdict read directly from `build/verification/*.json`, per `.agent/WORKFLOW.md`'s Routing Verification Gate. This plan is now complete.
...
This plan is complete. A* single-net search now has an explicit `SingleNetSearch` trait (`src/astar.rs`) that a future alternative search algorithm could implement, satisfying `.agent/PROJECT_GOAL.md`'s Future Architecture Initiative goal for this stage -- purely additively, with zero production call sites migrated, matching the low-risk posture that worked for the grid-snapping plan's `ObstacleMapBuilder` `Protocol`. Along the way, a real, verified duplication (`CrossingSearchConfig` construction repeated 3 times in `src/py_router.rs`) was found and eliminated behind a new helper mirroring the existing `astar_config` pattern.

This plan's own Milestone 0 characterization (dispatched to a fork, then independently spot-checked by Claude against the real source before being trusted) corrected the immediately-prior stage-characterization plan's claim in a meaningful way: the "4 clean entry points" framing undercounted the real public surface by more than half (10 functions, not 4, forming a genuine mode x stats-reporting matrix) -- while also confirming that same characterization's "no Python-side reimplementation" claim held up exactly as stated. This is useful evidence for how to weight future characterization claims generally: some hold up under a fuller check, some don't, and the only way to know which is to actually check, not to assume a prior "looks clean" pass in one direction or the other.

Two real findings surfaced during Milestone 0 were deliberately left out of this plan's scope, both flagged for a future pass: (1) `route_single_net_and_commit_native` (`src/py_router.rs:5491`+) reimplements the simple-then-full search fallback pattern inline instead of reusing the entry points that already have it -- a smaller instance of the same "god-method interleaving orchestration and algorithm" shape already parked for `route_many_with_repair_and_commit`; (2) `SimpleRouteObstacleQuery` (`src/simple_routes.rs`) was confirmed to already be correctly-scoped prior art and was deliberately left untouched, not folded into the new trait.

This plan's implementation work was also a useful test of the Claude+Codex flow under a genuine specification drift: the task file's draft trait signature for `search_with_collision_crossing` did not match the real underlying function's signature (an extra parameter, a different return shape), and Codex correctly detected this, adapted the trait method to match reality, and reported what it did rather than either forcing the wrong signature or silently guessing -- exactly the role-boundary behavior `.agent/CLAUDE_CODEX_FLOW.md` and this session's own prior experience (see `.agent/REPOSITORY_STATE.md`'s Recent Session Notes) predict.

Next step for the Future Architecture Initiative, per the recommended order: geometry realization (`src/geometry_realization.rs`), which the stage-characterization plan found is coupled at the type level with path-length matching (`src/plm.rs` imports meander-geometry types directly from it) -- that boundary needs a deliberate decision before either stage extracts cleanly, likely making the next plan's Milestone 0/1 more design-heavy than either of the two extractions completed so far.
