...
**Owner decision 2026-09-08: `plan-crossings-hybrid` IS contribution 1's default net order** (`default_net_order(guided=True)`; the flow passes `guided = enable_crossings and is_guided_mode(crossing_mode)`, so lidar-pure keeps `topological` and grid mode keeps `topological-span`). Re-run with the bare `--crossing-mode lidar-guided` flag: mm8 1 280 318 / 111/0/0 / 33, benes8 190 890 / 48/0/0 / 16, mm16 5 335 411 / 223/0/0 / 63, benes16 10 342 960 / 129/1/0 / 88, mm32 45 996 755 / 457/0/5 / 121, benes32 124 581 451 / 340/16/2 / 416 (629 s wall) -- the hybrid table exactly, all verifications 0 errors; benes_8x8 lidar-pure unchanged (52/4/0, 16). Contribution 1's paper claim is now: fewer crossings (mm16 65 -> 63), fewer failures and 18-95 % fewer expansions on the ladder, identical layouts with 34-45 % fewer expansions on the 32x32 cases. Earlier status: strictly better than S1 on every ladder benchmark and identical on the 32x32 cases -- a safe candidate for contribution 1's default net order. Was left opt-in because: making it the default changes contribution 1's ladder layouts (mm16 65 -> 63 crossings), which is the owner's call for the paper (the S1 claim "identical layouts to the baseline with fewer expansions" would become "fewer crossings, fewer failures and fewer expansions"). The net-282 harness investigation is not needed for this rule and stays open.
