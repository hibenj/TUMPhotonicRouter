...
Open questions added 2026-09-23 (Milestone 4): D7 should the global rip-up's epoch reset fire at every second global rip-up as the comment says, or only at the second as the code does? The plan keeps the code until answered. D8 the two mid-pipeline repair passes (final crossing repair, issue-group repair) are re-pointed from the legacy loop to the negotiated loop in Milestone 4 Slice 1; they never ran on a paper cell, so the gate cannot see them; confirm or ask for them to be deleted instead.
...
Acceptance: `loop_.rs` under 250 lines including comments; no `route_many_with_repair_and_commit`, `route_many_normal_and_commit`, `window`, `collision` (as modes), `allow_only_expected_pairs == true` paths or `use_collision_crossing_routing` left in `src/` or `translation/`; test baseline OK with the new tests counted; `gate_short.sh` 9 of 9 exact after every slice; the full 27-cell reproduction identical at the end of the milestone.
