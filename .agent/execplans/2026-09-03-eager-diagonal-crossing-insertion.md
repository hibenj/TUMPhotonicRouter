...
- OPEN DECISION (owner): eager extension through the X on the APPROACH move (the plan's original Milestone 1 and the still-failing test `perpendicular_diagonal_x_crossing_is_recorded_eagerly`). Evidence now says it is not needed for correctness: the move that CONTAINS the half-cell X records the crossing (`crossing_move_detects_offset_diagonal_halo_contact` proves it, and the margin-5 consecutive test routes the straight line through such an X). Options: (a) implement it anyway for robustness; (b) drop it, delete that test (or invert it to pin "approach move rejected, containing move accepts"), and remove the dead defer (Milestone 2). Claude's recommendation: (b) -- smaller kernel, no second mechanism, behaviour already correct and tested.
- OPEN DECISION (owner): set the after-crossing debt from `crossing_half_size_cells` (2) instead of `required_margin` (5), per the owner's 2026-09-03 clarification. Orthogonal to the fix above; would need its own ladder pass. Not done.
