...
Status note added 2026-08-17, after an audit prompted by the pre-existing `.agent/` docs having drifted during prior Codex-only sessions (see `.agent/REPOSITORY_STATE.md`'s "Recent Session Notes" for the fuller story): this plan is PAUSED, not complete and not abandoned. `.agent/ORCHESTRATOR.md` currently names `.agent/execplans/2026-08-11-refactor-python-routing-flow.md` as the active plan, itself a step toward a broader codebase-readability restructuring the user asked for on 2026-08-17. This plan should become active again once that work reaches a point where crossing-verification work can resume. Two concrete audit findings, so a future agent does not have to rediscover them: (1) the `Progress` checklist below is well-maintained through its last entry at 2026-07-17 19:10, but three earlier unchecked items look superseded by the 2026-07-16 "first full green `multiportmmi_8x8`" entry and were never reconciled — see the inline audit notes next to those checkboxes; this was not independently re-verified against current code, only checked for internal consistency against the plan's own later entries. (2) Exactly one commit postdates this plan's last revision note: `a29dc00` ("routing: mark stable dense benchmarks", 2026-08-11), which is not reflected in `Progress` at all; a new entry has been added for it below, sourced from `.agent/REPOSITORY_STATE.md`'s prior (now-trimmed) history rather than independently re-verified. Separately, the `Interfaces and Dependencies` section below contains roughly 2,100 lines of dated "Revision note" engineering history that belongs in `Progress` or `Surprises & Discoveries`, not under an interfaces header; this is real, valuable technical detail and was left in place rather than risk mangling it, but it is structural debt a future cleanup pass should fix.
...
- [ ] Repair the `multiportmmi_8x8` `n_32` cluster routing failure exposed by stricter native validation; current repair finds partial legal sub-attempts but cannot converge on a globally valid `n_32`/`n_35` pair reroute.
      Audit note, 2026-08-17: left unchecked in the source file, but the 2026-07-16 entry below reports a full green `multiportmmi_8x8` run (111/111 routed, 0 repairs), which is inconsistent with this cluster still being unresolved. Likely superseded and simply never reconciled; not independently re-verified in this audit.
...
- [ ] Solve the remaining dense `multiportmmi_8x8` routing strategy blocker: strict native crossing validation now stops at `n_51` rather than accepting a non-perpendicular/bend-footprint crossing.
      Audit note, 2026-08-17: same inconsistency as the `n_32` item above; the 2026-07-16 full green `multiportmmi_8x8` entry implies this was resolved too. Not independently re-verified in this audit.
...
- [ ] Continue from the clean invariant boundary: when Layer 1 rejects illegal crossing moves and cannot find a legal route, repair/rip-up is the intended next phase. Future work should improve legal route discovery or repair convergence, not reintroduce post-route illegality acceptance.
      Audit note, 2026-08-17: phrased as open-ended future work rather than a specific blocker; the many stabilization/optimization commits between 2026-07-16 and 2026-07-17 (repair guardrails, crossing hot-path optimization, dense routing stabilization) suggest this was substantially acted on. Left unchecked since it is not a single closeable fact; not independently re-verified in this audit.
...
- [x] (2026-08-11, commit `a29dc00`) Marked `benes_8x8`, `benes_16x16`, `multiportmmi_8x8`, and `multiportmmi_16x16` full-run stable with explicit `STABLE_ROUTING_ENV`/`STABLE_ROUTING_FLAGS` markers in their benchmark files; `multiportmmi_8x8` records `PHOTONIC_ROUTER_FANOUT_STUB_BEND_DEGREES=90` explicitly. `multiportmmi_32x32` is intentionally not marked stable; the last recorded evidence reaches `--debug-stop-after-route 155`, with route 156 / `n_155` as the next slow/hanging route to investigate. Audit note, 2026-08-17: this entry is reconstructed from `.agent/REPOSITORY_STATE.md`'s prior history and the commit message/diff, not independently re-verified by re-running the benchmarks in this audit.
...
Root cause:
    ...
    - A* could accept a post-crossing pending-margin state without preserving
      the direction in which the remaining runout had to continue.
    - It could also treat dynamic contacts generated by bend primitives as
      crossing candidates, even though the intended routing model requires
      crossing events to be produced by straight primitives only.

Fix:

    - Carry `pending_after_crossing_angle` in the crossing A* key/outcome and
      reject follow-up primitives whose initial runout does not continue in
      that angle.
    - Reject dynamic collisions on non-straight primitives before crossing
      classification.
    - Add focused Rust fixtures for wrong follow-up direction and internal
      bend-kink-before-after-margin cases.
...
Validation:
    ...
    $env:RUSTFLAGS='-C linker=rust-lld'
    $env:PYO3_PYTHON='C:\Users\benja\Documents\Repositorys\TUMPhotonicRouter\.venv\Scripts\python.exe'
    C:\Users\benja\.cargo\bin\cargo.exe +stable-x86_64-pc-windows-gnullvm test --no-run
    # passed; running the produced gnullvm test binary is still blocked by
    # STATUS_DLL_NOT_FOUND in this Windows runtime.

    $env:RUSTFLAGS='-C linker=rust-lld'
    $env:PYO3_PYTHON='C:\Users\benja\Documents\Repositorys\TUMPhotonicRouter\.venv\Scripts\python.exe'
    C:\Users\benja\.cargo\bin\cargo.exe +stable-x86_64-pc-windows-gnullvm rustc --release --features pyo3/extension-module --lib --crate-type cdylib
    Copy-Item target\release\photonic_router.dll python\photonic_router\_rust.pyd -Force
    .\.venv\Scripts\python.exe routing_flow.py multiportmmi_8x8 --crossings true --crossing-mode lidar-pure --debug-stop-after-route 36 --debug-svgs false --attempt-diagnostics
    # passed in the focused trace run; stop-after-36 crossing verification
    # success=True, error_count=0, crossing_count=7; photonic verification
    # success=True.

Report checks:

    - final internal net-36 (`n_35`) collision-crossing events include
      partners 35, 32, 33, and 34.
    - Rust crossing validation reports `satisfies=true` and
      `realized_violations=[]`.
    - Focused trace:
