...
## Milestone 6: validate against the real benchmark

Run `multiportmmi_8x8` under bare CLI defaults after Milestone 5's fix lands and is reviewed. Read the actual outcome directly (does `n_70` now route, or does it still fail -- and if it still fails, does it fail with a *different* signature, meaning the fix engaged but the combination still didn't work, or the *same* signature, meaning something about the fix or the reproduction did not work as designed). Record the real result in Outcomes & Retrospective, not an assumption.
