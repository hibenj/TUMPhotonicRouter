...
- **Grid-snapping is duplicated a third time, not implemented once per language as the characterization plan assumed.** See Purpose/Big Picture above for the exact locations and formulas. This was found while scoping this plan (2026-08-19), by directly grepping for `physical_to_grid`/`grid_cell_center` call sites outside `python/photonic_router/static_obstacle_builder.py` and reading `translation/route_rust.py:1416-1420`'s `_grid_cell_center_um` alongside `python/photonic_router/static_obstacle_builder.py:826-830`'s `grid_cell_center` -- both compute `origin + (cell + 0.5) * grid_size_um`, confirmed textually identical in formula (not yet confirmed identical in every call-site's actual argument values -- that is Milestone 0's job).
