...
- [x] Milestone 4 (broad validation) complete. Full ladder run by Claude directly (2026-08-19): `cargo test --lib` `330 passed, 0 failed` (already independently confirmed during Milestone 2 review). `PYTHONPATH=. .venv/bin/pytest -q` `21 failed, 325 passed, 1 skipped` -- failure-name set byte-identical to the pre-plan baseline (`21 failed, 322 passed, 1 skipped`), pass count difference exactly accounted for by the 3 new tests. `benes_4x4`: **Verdict: PASS** (`error_count=0, warning_count=0`, both crossing and photonic verification). `multiportmmi_8x8` bare CLI defaults: fails with the exact same pre-existing `RuntimeError: No route found for n_70` signature (`candidate_blockers=[70]`, identical `recent_errors` text) already root-caused and parked in the closed `2026-08-19-fix-open-repair-and-dense-port-findings.md` plan -- confirmed not a regression by exact string comparison. `multiportmmi_8x8` under its documented stable-baseline config (`STABLE_ROUTING_ENV`/`STABLE_ROUTING_FLAGS` from `benchmarks/multiportmmi_8x8.py`): **Verdict: PASS** (`error_count=0, warning_count=0`, both crossing and photonic verification). Every verdict read directly from `build/verification/*.json`, per `.agent/WORKFLOW.md`'s Routing Verification Gate. This plan is now complete -- see Outcomes & Retrospective.
...
This plan is complete. The first concrete extraction step of the Future Architecture Initiative landed cleanly and with zero behavior change: obstacle-map-building now has an explicit `ObstacleMapBuilder` `Protocol` formalizing a contract that was already real (two genuine swappable implementations), and grid-snapping's arithmetic -- previously hand-duplicated across at least 8 locations in 7 files spanning both languages, including one call site with a silently different rounding rule -- now has exactly one canonical implementation per language (`floor_snap_to_grid`/`cell_center_coordinate` in `src/static_obstacle_builder.rs` and the matching pair in `python/photonic_router/static_obstacle_builder.py`), with every other call site migrated to call it. The genuinely-different round-to-nearest functions and the genuinely-different no-offset conversion were correctly identified and left untouched throughout.

What actually happened versus what was originally scoped: Milestone 0's initial characterization (from the prior stage-characterization plan) described this pair of stages as already "cleanly separable," implying this plan would be close to a documentation exercise. Scoping this plan's own Milestone 0 found that was true for obstacle-map-building but not for grid-snapping -- the duplication footprint was far larger than first assumed (found in three separate passes: an initial "one more implementation," then a fuller Python-side grep revealing ~10 sites, then a Rust-side grep revealing the same pattern there too). This is itself a useful data point for future stages in this initiative: "no duplication found" claims from a characterization pass should be read as "no duplication found in the specific named functions checked," not as an exhaustive guarantee -- formula-level duplication (the same arithmetic re-derived inline without a shared name) is easy to miss with a symbol-name-based search and needs a formula-pattern grep to catch.

The Claude+Codex flow worked as intended for both implementation slices: two independently-scoped, well-specified task files (Rust call-site migration, Python call-site migration + `Protocol` addition) were dispatched, both came back clean on their own validation ladders, and Claude's review caught one real (if minor) code-quality issue -- a throwaway `GridSpec(width=0, height=0, ...)` construction in the Python delegation wrappers -- that was fixed directly rather than via a third Codex round-trip, given its size, and re-validated before committing. This is recorded as a normal, expected part of the reviewer role, not a failure of the delegation.

The two Codex tasks were run sequentially rather than in parallel, deliberately, given this machine's demonstrated resource constraints earlier in the session (a `multiportmmi_16x16` diagnostic run that had to be killed after 10 minutes). Future multi-slice dispatches on this machine should default to sequential unless the slices are both small/fast, not assume parallel dispatch is free.

Next step for the Future Architecture Initiative, per the stage-characterization plan's recommended order: A* single-net search (`src/astar.rs`'s entry points) -- already clean and well-tested per that plan's own findings, the next-lowest-risk target.
