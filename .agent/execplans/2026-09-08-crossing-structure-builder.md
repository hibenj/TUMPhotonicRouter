...
- [x] (2026-09-08) Step 2, router fallback: a layer the selector cannot place keeps its original nets (`DerivedCrossingLayout.router_fallback_net_names`, `CrossingGridGeometry.router_fallback=True`; False restores the hard stop). The flow then runs the routing stage with crossings enabled in `lidar-guided` mode and hands the fallback nets down as `crossing_guidance_net_names` (flow -> `OpticalRoutingStageConfig` -> `route_match_and_realize` -> `route_nets_rust` -> session -> `_build_crossing_plan_info(guidance_net_names=...)`), which loads only plan events between two of those nets: the tiled layers' segments never see a planned pair and keep the full 200 um price, i.e. they stay crossing-free in practice. Test knob `PHOTONIC_ROUTER_CROSSING_GRID_ROUTER_LAYERS=<source depths>` forces layers to the router.
- [x] (2026-09-08) Step 3, mixed-run verification: `routing_flow_verification._check_mixed_run_crossing_budget` -- pre-placed tiles + crossings realized by the router == the plan's total (`plan_event_count`), printed and enforced. multiportmmi_8x8 with layer 6 forced to the router: 26 tiles + 7 router crossings = 33 of 33, both verifications 0 errors, guidance `planned_pairs=7`. `tests/test_router_fallback.py` pins it; grid ladder of the seven benchmarks unchanged (no fallback there).
