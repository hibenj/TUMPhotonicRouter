...
- Observation (2026-08-26, owner GDS review of 4x4): two more defects, both with exact root causes. (1) The lane that crosses nothing detoured 35 um around the grid: not the grid's bounding box (its row was statically free) but the generic per-port lane reservation of the grid ports (`port_lane_length_cells=8`, `half_width=4`), which covers a sibling row 1.25 um away. Fixed with a zero-size port access rule for grid components (new `register_component_port_access_rule` hook in `python/photonic_router/routing_layers.py`), the same reasoning that zeroed static-stub reservations earlier. (2) Every from-grid stub carried a 12 um four-bend bump although source and target rows agreed to 1 nm: `full_straight_offset_bump_candidates` generates bumps whenever `|dy| > EPS`, and the checked correction path (`route_port_corrected_centerline_checked_and_commit_native`, `src/py_router.rs`) tries those candidates *before* the ordinary strategy chain that starts with the plain straight shift. The 1 nm came from chaining the exit straight via `connect` (dbu rounding); fixed by snapping the grid placement to dbu and placing exit straights absolutely. 4x4 now has 8/8 straight stubs, 0 bumps, 0 detours. Pushing grid ports away from unrouted siblings (tried at 4 um) only converts detours into bumps -- knob kept, default 0.

- Observation (2026-08-26, remaining corrector gap, NOT fixed): sibling pairs that are both in a grid have ports spread 4 um apart at the grid face but 1.25 um apart at the switch, so each stub needs a 1.38 um lateral offset on an otherwise straight route. The corrector realizes every such offset as a 13.4 um four-bend bump (16 in 8x8, 80 in 16x16) because for a full straight it has no small-offset S-bend strategy -- only the bump. Options: (a) add an S-bend (two opposite shallow bends / Euler S) strategy to `src/geometry_realization.rs` tried before bumps and make sub-dbu port mismatches count as equal; (b) reorder the checked path to try the strategy chain before bump candidates; (c) accept. Repository owner's stated preference ("bumps only if the first is not usable") points to (a)+(b); not started -- it is a corrector change outside this plan's grid scope.

- Observation (2026-08-26, late): the in-grid fan-in was rejected by the repository owner as a matter of principle ("the stubs are already taking away basically the routing ... I only want to show that we can take away the crossing complexity"). Grids now default to the lattice-only core (`fan_mode="router"`), and the router does the fan-out. Result: `benes_4x4` and `benes_8x8` clean with **zero** bumps, detours and overlaps (`legacy-runway`); `benes_16x16` fails again at a switch output pair (`sw_s0_6,o4 -> in_12`): both outputs of one 2 um cell must climb the same way toward the compact core and the first seals the second. Two experiments toward the owner's suggested fix ("straights from the middle outward like the multiport MMI stubs, route inside out") did not transfer: (1) `--fanout-access-mode static-stubs` makes the router build its own staggered stubs on the grid ports and its target-anchor endpoint correction then fails on `benes_8x8` (`port endpoint correction would require an unsupported terminal stub`, `in_0`); (2) lowering the dense-fanout threshold to 2 so switch pairs get two-bend spreading stubs (temporary env knob, reverted) still fails 16x16 at the target sibling, and excluding grid ports from dense detection would need the anchors pass reordered after access rules are resolved. Both are router-side changes and were not pursued inside this plan.

- Observation (2026-08-26, latest): staggered grid stubs (middle lanes longest, 6 um per rank; 12 um tried too) resolve the *source*-side switch-pair failure in `benes_16x16` (net 73) but not the mirrored *target*-side one: net 134 `crossing_grid_stage1_run0,out_2 -> sw_s1_1,o1` fails with `corridor_clearance_target_region_size=1` and a dynamic overlap right in front of the port -- the sibling input `o2`'s route (net 130), which arrived first from the same side and occupies the shared 2 um cell. A stagger at the grid cannot influence the switch side. Partial artifact for inspection: `build/routed_benes_16x16_before_failure.gds` (debug-stop after route 133, not a full result). Next step is router-side: dense-target handling for 2-port switch input pairs (staggered target runways/stubs, threshold currently hard-coded `> 2`), with grid ports excluded from automatic stub/runway generation.

- Observation (2026-08-26, owner GDS review of the 16x16 partial): the "overshoot then come back" hooks on the edge-lane stubs are a router cost artifact, not a grid or self-collision problem. Cell-level map of the committed routes (`build/routed_benes_16x16_before_failure.gds`, routes A/B/C = `n_s0_0_o1`, `n_s0_1_o0`, `n_s0_1_o1` to-grid stubs): the three arrive from the switches as a tight 45-degree bundle **3 cells apart** (the A* cost favours diagonals), each then needs ~7 cells (3-cell bend + run-in) to turn into its port, so each descends one cell short of its port column and jogs 6 cells west. Stub stagger 6/12/18 um leaves the 12 hook routes unchanged (the bundle pitch is set by the incoming diagonals, not the ports). No self-intersecting centerline exists in any of the 133 records (raw or corrected); at stagger 12 two *pairs* of stubs cross each other in the partial artifact (`n_s0_6_o1`/`n_s0_7_o0` and the stage-6 mirror), which a full run would reject. Routing inside-out (grid port as source, tried via a temporary direction flip, reverted) fails earlier (net 63) at the same switch pair, now as a target. Conclusion: whichever direction, the invariant blocker is the switch's two ports sharing one 2 um routing cell while both must connect toward the same side; that is router-side dense-pair handling (2-port target/source groups, the `> 2` threshold), outside this plan.

## Decision Log

- Decision: the grid contains only the crossing lattice (plus 4 um straight stubs at its ports); no fan-in geometry. The `in_grid` mode stays available behind `PHOTONIC_ROUTER_CROSSING_GRID_FAN_MODE=in_grid` as reference.
  Rationale: repository owner's explicit direction, above.
