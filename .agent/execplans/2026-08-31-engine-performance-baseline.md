...
- [x] (2026-08-31) Milestone 1, second fix landed (cf3f3f1): the unresolved 16.9% `0x...b00` profile entry was the **vDSO** -- `clock_gettime` from the per-expansion micro-timers (legality/heap/neighbor), which run by default because `SCRIPT_DEBUG_TIMING=True` switches `collect_detailed_timing` on. They now additionally require `PHOTONIC_ROUTER_HOT_LOOP_TIMING`; per-attempt/per-search timers (the standard reports) unchanged. Ladder 0/0, pytest byte-identical baseline. Measured: mm_16x16 133.5 -> 122.2 s, benes_16x16 239.9 -> 217.3 s, **heater_s_mod ~25 -> 8.2 s**, pytest suite 99 -> 58 s. Note: profile had shown ~17% vDSO but realized win is ~8-9% on the big benchmarks -- perf inflates clock_gettime cost with its own overhead; recorded so nobody re-counts it. Cumulative today: **multiportmmi_16x16 200 -> 122.2 s (-39%)**.
...
- [x] (2026-08-31) Milestone 1b, balance sheet measured (`--verbose-routes --debug-timing true` on both 16x16): **every repair strategy that fires converts 100%** -- multiportmmi_16x16: probe_route 72/72 ok (15.3 s), repair_failed_net 16/16 (3.4 s), reroute_victims 8/8 (1.7 s); benes_16x16: probe 16/16 (20.0 s), zero classic repair rounds. Nothing to delete at this scale. The real waste, quantified: (a) the fail-then-probe pattern -- 73 (mm16) / 24 (benes) normal_route searches run to exhaustion ("No legal LiDAR crossing route found") and the probe fallback then succeeds EVERY time; detecting legal-crossing infeasibility early (or reordering probe-first for repeat offenders) is the macro lever, but strategy reordering is owner-gated -- goes to the decision list. (b) benes's giant single searches (up to 2.76M expansions / 11.75 s per net): the search front balloons because each crossing costs SEARCH_LOSS=200 um-equivalent, exactly the mechanism the owner's crossing-guided-A* contribution addresses; E4 measures the sensitivity. Original scope: mine the existing per-attempt stats (`--debug-timing` / `collect_route_stats` timing buckets: attempts, successes, wall time and expansions per bucket -- `normal_route`, `probe_route`, `reroute_victims`, `repair_failed_net`, `lidar_pure_probe_commit`, ...) on the green ladder into a strategy balance sheet: fires how often, converts how often, costs how much per conversion. Strategies with high cost and low conversion become candidates for reordering, gating, or removal -- presented to the owner as options, not changed unilaterally (repair-strategy changes are decision-gated by standing practice).
...
- [ ] Candidate (designed 2026-09-01, owner-approved to note here; do AFTER the mm32 net-156 legality work): **lazy post-crossing completion at pop time.** The eager-completion chain (fix `87df14c`, `.agent/execplans/2026-09-01-forced-90-degree-route-degradation.md`) completes every accepted crossing's straight debt at successor-GENERATION time, including for entries A* never pops -- lost laziness. Design: enqueue a light "unchained" heap entry with an admissible f (g + minimal completion base cost + heuristic from the projected debt-end cell; history/congestion >= 0 keeps it a lower bound), run the chain once at POP, discard on failure. Keeps the state-space fix (no debt-carrying keys) while skipping chains for unpromising crossings. Expected gain: small on mm16 (~3-4 s search-side); potentially large in crossing-dense searches (mm32 net-156 class generated 1.37M pending events, mostly never popped). NOT expected to recover mm16's ~30 s repair-attempt drift (tie-break lottery, see the knife-edge n_133 finding in the 90-degree plan) -- that belongs to repair robustness. Every kernel perturbation needs the full ladder re-run and risks the mm16 n_133 knife edge; budget for that.
...
- E4 `COLLISION_CROSSING_SEARCH_LOSS_UM` 100/200/400 on `benes_16x16`:
  insensitive -- identical 88 crossings (topology-determined), 0 errors,
  195.9/200.7/202.6 s. Benign in the tested range; lowest priority.
- Window-scale follow-up: `multiportmmi_16x16` at 0.05 routes clean at
  **97.8 s (vs 114 s pinned 0.35)**, `multiportmmi_8x8` at **21.8 s (vs
  31.3 s)**; both 0.35 pins removed (ca3a56f).

### Round verdict (revisit before 32x32, complete)

Of the audit's top suspects: one was the already-fixed root problem
(heuristic weight), two are load-bearing and now evidenced (bend weight,
tie flip), one resolved as a pure-cost misconfiguration and removed
(window pins), one is benign in range (search loss). Repair strategies:
nothing to delete -- each converts 100%; the two real levers are
owner-gated (fail-then-probe reordering) or contribution territory
(crossing-guided search vs the 200 um search-front ballooning).
