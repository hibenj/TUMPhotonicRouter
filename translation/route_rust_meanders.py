...
class _MeanderRouterProtocol(Protocol):
    ...
    def register_meander_route_cells_as_static(
        ...
        route_clearance_radius_cells: int = 0,
...
@dataclass
class _LegacyRequirementAttempt:
    selected_candidate: DelayInsertionCandidate | None
    selected_candidate_requested: float
    selected_affected_edges: tuple[RoutedEdgeKey, ...]
    ...
    bundle_candidate_calls: int
    bundle_edge_calls: int
    bundle_planned: int
    bundle_no_candidate: int
    fallback_candidate_calls: int
    fallback_edge_calls: int
    last_exc: Exception | None
    max_bumps: int
    opened_route_cell_count: int
...
class _MeanderPlannerContext:
    ...
    ) -> tuple[bool, set[GridCell]]:
        blocked_cells = set(self.base_static_cells)
        blocked_cells.update(self.reserved_meander_cells_for_python_planning())
        if extra_blocked_cells:
            blocked_cells.update(extra_blocked_cells)
        rect_cells = _grid_rect_cells(parsed_grid_rect)
        overlap = rect_cells & blocked_cells
    ...
    def plan_edge_candidate(
        ...
        *,
        ...
        blocked, overlap_cells = self._candidate_grid_rect_overlaps_blocked(
            candidate_selected_grid_rect,
            extra_blocked_cells=extra_reserved_cells,
        )
        if blocked:
            attempt_info["reason"] = (
                f"candidate selected_grid_rect overlaps blocked cells ({len(overlap_cells)} cells)"
            )
            attempt_info["rejected_box_blocked"] = True
            attempt_info["blocked_overlap_count"] = len(overlap_cells)
            return (
                attempt_info,
                None,
                record,
    ...
    def plan_candidate_bundle_registered(
        self,
        *,
        candidate_edge_keys: tuple[RoutedEdgeKey, ...],
        requested: float,
        box_depths_um: list[float],
        min_straight_um: float,
        min_seg_um: float,
        max_height_um: float,
        endpoint_inset_um: float,
    ) -> CandidateBundleAttempt | None:
        if not hasattr(
            self.router,
            "plan_auto_analytic_meander_candidate_bundle_registered_opened",
        ):
            return None
        ...
        def bundle_failure(
            *,
            ...
            edge_calls: int | None = None,
        ) -> CandidateBundleAttempt:
            return (
                True,
                [],
                edge_attempts,
                ...
                elapsed_s,
                last_exc,
                len(edge_attempts) if edge_calls is None else edge_calls,
            )
        ...
        for candidate_edge_key in candidate_edge_keys:
            ...
            max_bumps_by_edge.append(candidate_max_bumps)
            edge_attempts.append(attempt_info)
            open_cell_count += _as_int(
                attempt_info.get("opened_route_cell_count"),
                0,
            )

        t_plan_start = time.perf_counter()
        bundle_result: dict[str, object] | None = None
        last_exc: Exception | None = None
        try:
            bundle_result = cast(
                dict[str, object],
                self.router.plan_auto_analytic_meander_candidate_bundle_registered_opened(
                    centerlines,
                    registered_indices,
                    requested_extra_length_um=float(requested),
                    box_depths_um=box_depths_um,
                    max_bumps_by_edge=max_bumps_by_edge,
                    min_bend_radius_um=None,
                    min_straight_um=min_straight_um,
                    max_meander_height_um=max_height_um,
                    min_segment_length_um=min_seg_um,
        ...
        if bundle_result.get("status") != "planned":
            ...
            if 0 <= failed_index < len(edge_attempts):
                edge_attempts[failed_index]["reason"] = str(
                    bundle_result.get("reason", "")
                )
            return bundle_failure(
                elapsed_s=elapsed_s,
                edge_calls=min(len(edge_attempts), failed_index + 1),
            )

        raw_plans = bundle_result.get("plans", [])
        if not isinstance(raw_plans, list) or len(raw_plans) != len(records):
            if edge_attempts:
                edge_attempts[0]["reason"] = "registered bundle planner returned invalid plan count"
            return bundle_failure(elapsed_s=elapsed_s)

        candidate_plans: list[PlannedEdgeInsertion] = []
        candidate_reserved_cells: set[GridCell] = set()
        for edge_key, record, rr_obj, attempt_info, candidate_max_bumps in zip(
            candidate_edge_keys,
            records,
            raw_plans,
            edge_attempts,
            ...
            if not isinstance(rr_obj, dict):
                attempt_info["reason"] = "registered bundle planner returned invalid plan"
                return bundle_failure(elapsed_s=elapsed_s)
            ...
            if parsed_grid_rect is None:
                attempt_info["reason"] = "registered bundle planner returned invalid selected_grid_rect"
                return bundle_failure(elapsed_s=elapsed_s)
            blocked, overlap_cells = self._candidate_grid_rect_overlaps_blocked(
                parsed_grid_rect,
            ...
            if blocked:
                attempt_info["reason"] = (
                    "registered bundle planner selected_grid_rect overlaps blocked cells"
                )
                attempt_info["blocked_overlap_count"] = len(overlap_cells)
                attempt_info["rejected_box_blocked"] = True
                return bundle_failure(elapsed_s=elapsed_s)
    ...
    def commit_planned_edge(
        ...
        *,
        ...
        if parsed_grid_rect is None:
            return
        blocked, _overlap_cells = self._candidate_grid_rect_overlaps_blocked(
            parsed_grid_rect,
...
def _build_planner_context(
    *,
    ...
    route_cells_by_edge: dict[RoutedEdgeKey, set[GridCell]] = {}
    route_clearance_radius_cells = max(0, int(route_clearance_radius_cells))
    can_use_registered_route_cells = hasattr(
        router,
        ...
        for edge_key, record in by_edge.items():
            ...
            if route_clearance_radius_cells > 0:
                route_cells = _inflate_grid_cells(
                    route_cells,
                    radius_cells=route_clearance_radius_cells,
    ...
    setup_profile = {
        ...
        "route_clearance_radius_cells": float(route_clearance_radius_cells),
...
def _plan_requirement_with_legacy_candidate_loop(
    *,
    context: _MeanderPlannerContext,
    work_items: list[_CandidateWorkItem],
    search_config: _MeanderSearchConfig,
    candidate_profile: dict[str, dict[str, object]],
) -> _LegacyRequirementAttempt:
    """Compatibility path for backends without registered requirement batching."""
    ...
    for work_item in work_items:
        ...
        if bundle_attempt is not None:
            ...
            planning_elapsed_s += candidate_elapsed_s
    ...
    return _LegacyRequirementAttempt(
        selected_candidate=selected_candidate,
        selected_candidate_requested=selected_candidate_requested,
        selected_affected_edges=selected_affected_edges,
        ...
        bundle_candidate_calls=bundle_candidate_calls,
        bundle_edge_calls=bundle_edge_calls,
        bundle_planned=bundle_planned,
        bundle_no_candidate=bundle_no_candidate,
        fallback_candidate_calls=fallback_candidate_calls,
        fallback_edge_calls=fallback_edge_calls,
        last_exc=last_exc,
        max_bumps=max_bumps,
        opened_route_cell_count=opened_route_cell_count,
    )
