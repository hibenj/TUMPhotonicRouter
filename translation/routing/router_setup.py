...
def configure_router_and_grid(session, obstacle_map: Any) -> None:
    ...
    if session.config.diagnostics.trace_grid:
    ...
    session.astar_cfg = session.rust_backend.AStarConfig(max_iterations=int(session.max_iterations))
    ...
    session.astar_cfg.enable_simple_routes = bool(session.enable_simple_routes)
    session.astar_cfg.enable_jps4 = bool(session.enable_jps4)
    session.astar_cfg.use_indexed_heap = bool(session.use_indexed_heap or session.allow_45_degree_turns)
    ...
    session.astar_cfg.primitive_ordering = str(session.primitive_ordering)
    effective_heuristic_mode = str(session.heuristic_mode)
    if session.allow_45_degree_turns and effective_heuristic_mode == "heading_aware":
        effective_heuristic_mode = "diagonal_aware"
    session.astar_cfg.heuristic_mode = effective_heuristic_mode
    collision_crossing_mode = bool(session.enable_crossings) and is_collision_mode(
        session.crossing_mode
    )
    min_heuristic_weight = float(session.config.search.min_heuristic_weight)
    if (
        session.allow_45_degree_turns
    ...
    if session.allow_45_degree_turns and hasattr(session.astar_cfg, "heuristic_weight"):
    ...
    if session.allow_45_degree_turns and hasattr(session.astar_cfg, "bend_weight"):
        ...
        min_bend_weight = float(session.config.search.min_bend_weight)
        session.astar_cfg.bend_weight = max(float(session.astar_cfg.bend_weight), min_bend_weight)
    effective_heap_tie_breaker = str(session.heap_tie_breaker)
    if session.allow_45_degree_turns and effective_heap_tie_breaker == "smaller_g":
    ...
    env_tie_breaker = session.config.search.heap_tie_breaker
    ...
    if hasattr(session.astar_cfg, "proactive_congestion_weight"):
        session.astar_cfg.proactive_congestion_weight = float(session.proactive_congestion_weight)
    if hasattr(session.astar_cfg, "proactive_congestion_radius_cells"):
        session.astar_cfg.proactive_congestion_radius_cells = int(
            session.proactive_congestion_radius_cells
        )
    if session.routing_window_scale is not None:
        session.astar_cfg.routing_window_scale = float(session.routing_window_scale)
    ...
    session.router = session.rust_backend.PyPhotonicRouter(
        ...
        session.astar_cfg,
        session.router_config.to_rust(session.rust_backend),
    )
    if hasattr(session.router, "set_route_width_um"):
        ...
        session.router.set_route_width_um(float(session.route_width_um))
    ...
    session.stub_port_lane_length_cells = session._fanout_int_or_default(
        session.config.fanout.stub_port_lane_length_cells,
        0,
    )
    session.stub_port_lane_half_width_cells = session._fanout_int_or_default(
        session.config.fanout.stub_port_lane_half_width_cells,
        0,
    )
