...
def main() -> int:
    ...
    ap.add_argument(
        "--pitch", type=float, default=10.0, help="flag parallel neighbours closer than this (um)"
    )
    a = ap.parse_args()
        ...
        for sh in top.begin_shapes_rec(li):
            ...
            for x0, y0, x1, y1 in collinear_runs(poly):
                ...
                if a.dir == "v" and abs(dx) < 1e-6 and abs(dy) >= a.min_len:
                    ...
                    a.dir == "d" and abs(abs(dx) - abs(dy)) < 0.05 and abs(dx) >= a.min_len / 1.4142
                ):
    ...
    if a.dir == "d":
        ...
        for (x0, y0, x1), length in items:
            if (
                shown
                and abs(shown[-1][0] - x0) <= 0.6
                and abs(shown[-1][1] - y0) <= 0.6
                and abs(shown[-1][3] - length) <= 1.0
            ):
                continue
    ...
    for i, (pos, lo, hi) in enumerate(merged):
        ...
        flag = (
            f"  <-- parallel within {a.pitch:.0f} um (not both crossable by one straight): "
            + ", ".join(flags)
            if flags
            else ""
        )
